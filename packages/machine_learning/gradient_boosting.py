"""Deterministic chronological gradient-boosting challenger baseline."""
...
class GradientBoostingConfig:
    ...
    def __post_init__(self) -> None:
        if min(self.estimators, self.maximum_depth, self.minimum_samples_leaf) <= 0:
            raise ValueError("gradient-boosting size controls must be positive")
        if not 0 < self.learning_rate <= 1:
            raise ValueError("gradient-boosting learning rate must be in (0, 1]")
